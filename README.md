# 目录

## ide

### intellij idea

[一种可用的IDEA配置默认JVM参数方式.md](ide/intellij idea/一种可用的IDEA配置默认JVM参数方式.md)

## java

### jdk

[Java8一些暗坑](java/jdk/Java8一些暗坑.md) 

### jvm

[JVM学习笔记](java/jvm/JVM学习笔记.md)

### mybatis

[MybatisGenerator简单使用](java/mybatis/MybatisGenerator简单使用.md)

### spring

[Spring4学习笔记](java/spring/Spring4学习笔记.md)

[Spring Boot与普通Spring项目配置共享session](java/spring/Spring Boot与普通Spring项目配置共享session.md)

[spring cloud的部分优化方案](java/spring/spring cloud的部分优化方案.md)

[普通Spring项目配置Eureka通信](java/spring/普通Spring项目配置Eureka通信.md)

### util

[jdbcTemplate快速使用](java/util/jdbcTemplate快速使用.md)

[Maven常用知识总结](java/util/Maven常用知识总结.md)

[SnowFlake分布式产号算法](java/util/SnowFlake分布式产号算法.md)