...
- 64位架构下jvm内存分配不要超过32g
  - 在64位架构下，jvm内存配置在4g-32g时，由于超过了32位架构的限制，经优化jvm会采用压缩指针策略，简单来说就是通过偏移量来寻址，使用的仍是4字节的指针。
  - 超过32g时，不得不使用64位架构下的指针，需要8字节，因此开销会大很多，甚至会明显影响gc。通常消耗是原来的1.5倍，据说要40-50g的内存才有实际32g的效果。
